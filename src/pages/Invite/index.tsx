...
import { handleSetRedirectUrl } from "config/slices/auth";
...
const InvitePage = () => {
  ...
  React.useEffect(() => {
    if (!token) {
      dispatch(handleSetRedirectUrl(location.pathname));
      navigate("/");
    } else {
      axios
  ...
  const acceptInvitation = () => {
    ...
    axios
      .post(
        "/api/invites/accept",
        {
          serverId: invitation?.server._id,
        },
        {
          headers: {
            Authorization: `Bearer ${token}`,
            ...
            {invitation && invitation?.server ? (
              ...
              >
                ...
                <Typography.Text style={{ fontSize: "16px", color: "#fff" }}>
                  {invitation.inviter?.username} invited you to join
                </Typography.Text>
                ...
                {invitation.server?.users?.includes(currentUser?._id) ? (
                  <Button
                    type="primary"
                    block
                    disabled
